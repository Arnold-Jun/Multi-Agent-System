...
public class CallAgent implements NodeAction<AgentMessageState> {
     ...
     */
    public CallAgent( String agentName,  BaseAgent agent ) {
        this.agent = agent;
     ...
     */
    private Map<String,Object> mapResult( ChatResponse response, String requestId, String sessionId) {
...
//                textJSON.put("requestId", requestId);
                Map<String, String> map = Map.of("requestId", requestId, "sessionId", sessionId,"context", text);
        ...
        if (response.finishReason() == FinishReason.TOOL_EXECUTION || content.hasToolExecutionRequests() ) {
            ...
            if (toolInterceptor != null) {
                ...
                toolInterceptor.setSessionId(sessionId);
        ...
        if( agent.isStreaming()) {
            ...
            if (generator == null) {
                generator = StreamingChatGenerator.<AgentMessageState>builder()
                        .queue(queue)
                        .mapResult( res -> mapResult(res, requestId, sessionId) )
            ...
            return mapResult(response, requestId, sessionId);
        }
