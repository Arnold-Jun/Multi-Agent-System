...
import com.xiaohongshu.codewiz.codewizagent.agentcore.mcp.local.LocalServerToolProvider;
...
public class ToolProviderManager {
    ...
    public Optional<ToolExecutionResultMessage> execute(@NonNull ToolExecutionRequest request, Object memoryId, String username, String requestId) {
        if (request == null) {
            ...
            Optional<ToolExecutionResultMessage> result =
                    toolProviderResults.stream().flatMap(toolProviderResult -> toolProviderResult.tools().entrySet().stream())
                            ...
                            .findFirst().map((e) -> {
                                String value = e.getValue().execute(request, memoryId);
    ...
    public Optional<ToolExecutionResultMessage> execute(@NonNull Collection<ToolExecutionRequest> requests, Object memoryId, String username, String requestId) {
        if (requests == null) {
            throw new NullPointerException("requests is marked non-null but is null");
        } else {
            for(ToolExecutionRequest request : requests) {
                Optional<ToolExecutionResultMessage> result = this.execute(request, memoryId, username, requestId);
                if (result.isPresent()) {
                    return result;
                }
            }
