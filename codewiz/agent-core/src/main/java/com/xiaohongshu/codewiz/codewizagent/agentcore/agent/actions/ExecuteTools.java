...
public class ExecuteTools implements NodeAction<AgentMessageState> {
     ...
     */
    public ExecuteTools(@NonNull String agentName, @NonNull BaseAgent agent, @NonNull ToolProviderManager toolProviderManager, @NonNull String username, @NonNull String requestId) {
    ...
    public Map<String,Object> apply(AgentMessageState state )  {
        ...
        var result = toolExecutionRequests.stream()
                        .map(toolExecutionRequest -> toolProviderManager.execute(toolExecutionRequest, username, requestId))
