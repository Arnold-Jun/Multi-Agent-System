...
public class SubgraphStateManager {
     ...
     * 保存子图状态
     */
    public void saveSubgraphState(String sessionId, String agentName, String subgraphType, SubgraphState state) {
     ...
     * 加载子图状态
     */
    public Optional<Map<String, Object>> loadSubgraphState(String sessionId, String agentName, String subgraphType) {
        if (!checkpointConfig.getSubgraphPersistence().isEnabled()) {
    ...
    private SubgraphExecutionRecord extractPersistentState(String sessionId, String agentName, SubgraphState state) {
        ...
        // 提取工具执行历史
        ToolExecutionHistory history = state.getToolExecutionHistory();
        if (history != null) {
