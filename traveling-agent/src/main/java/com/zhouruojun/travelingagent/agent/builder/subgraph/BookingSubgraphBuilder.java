 ...
 */
@Slf4j
public class BookingSubgraphBuilder extends BaseSubgraphBuilder<SubgraphState> {
    ...
    protected String getPrompt() {
        return promptManager.getSubgraphSystemPrompt("bookingAgent");
    }
    ...
    protected StateGraph<SubgraphState> buildGraph(CallSubAgent callAgent, ExecuteTools<SubgraphState> executeTools) throws GraphStateException {
        ...
        return new StateGraph<>(SubgraphState.SCHEMA, stateSerializer)
                .addNode("bookingAgent", node_async(callAgent))
                .addNode("executeTools", node_async(executeTools))

                // 边连接
                .addEdge(START, "bookingAgent")
                .addConditionalEdges("bookingAgent",
