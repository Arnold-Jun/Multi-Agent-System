...
public class PushNotificationReceiverAuth extends PushNotificationAuth {
    ...
    public PushNotificationReceiverAuth loadJwks(List<String> jwksUrls) {
        ...
        List<String> errorJwksUrls = new ArrayList<>();
        jwksUrls.stream().forEach(jwksUrl -> {
            try {
                ...
                errorJwksUrls.add(jwksUrl);
            }
        });

        if (CollectionUtils.isNotEmpty(errorJwksUrls)) {
            jwksUrls.removeAll(errorJwksUrls);
        }

        if (jwkMap.isEmpty()) {
            log.warn("a2a.notification.jwksUrls are all not unavailable");
            return this;
//            throw new RuntimeException("a2a.notification.jwksUrls are all not unavailable");
        }
