...
import com.zhouruojun.agentcore.spec.error.ContentTypeNotSupportedError;
import com.zhouruojun.agentcore.spec.error.UnsupportedOperationError;
...
////是否允许解析使用Java/C++ 样式的注释（包括'/'+'*' �?//' 变量�?//        objectMapper.configure(JsonParser.Feature.ALLOW_COMMENTS, true);
//
//
////是否允许单引号来包住属性名称和字符串�?//        objectMapper.configure(JsonParser.Feature.ALLOW_SINGLE_QUOTES, true);
//
////是否允许JSON字符串包含非引号控制字符（值小�?2的ASCII字符，包含制表符和换行符�?//        objectMapper.configure(JsonParser.Feature.ALLOW_UNQUOTED_CONTROL_CHARS, true);
//
////是否允许JSON整数以多�?开�?//        objectMapper.configure(JsonParser.Feature.ALLOW_NUMERIC_LEADING_ZEROS, true);
//
////null的属性不序列�?//        objectMapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
//
////按字母顺序排序属�?默认false
...
//
////序列化枚举是否以toString()来输出，默认false，即默认以name()来输�?//        objectMapper.configure(SerializationFeature.WRITE_ENUMS_USING_TO_STRING, true);
//
...
//
    ...
    /// /序列化BigDecimal时是输出原始数字还是科学计数，默认false，即以toPlainString()科学计数方式来输�?//        objectMapper.configure(SerializationFeature.WRITE_BIGDECIMAL_AS_PLAIN, true);
//    }
//
